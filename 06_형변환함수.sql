...
SELECT TO_CHAR(20000, '99999') FROM dual; -- '99999' 다섯자리까지 표현
-- 주어진 자릿수에 숫자를 모두 표기할 수 없어서 모두 #으로 표기됩니다.
SELECT TO_CHAR(20000, '9999') FROM dual; -- '9999' 네자리까지 표현
SELECT TO_CHAR(20000.29, '99999.9') FROM dual;
SELECT TO_CHAR(20000, '99,999') FROM dual;

SELECT TO_CHAR(salary, 'C99,999') AS salary
FROM employees;
...
SELECT -- 순차적으로 진행되는게 아니라 동시에 진행이 되므로 별칭을 이용해서 검색할 수 없다.
    TO_CHAR('20050102', 'YYYYMMDD') AS date,
    TO_DATE(date, 'YYYY"년" MM"월" DD"일"')
FROM dual;
