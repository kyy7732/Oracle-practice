...
-- SQL은 대소문자 구문이 없지만 키워드는 대문자로 작성이 관례
SELECT
    *
FROM
    employees;
...
SELECT
    ...
    last_name,
    salary,
    salary + salary*0.1 AS 성과금 -- salary에 0.1을 곱한다
FROM employees;
...
-- alias(AS) (컬럼명, 테이블명의 이름을 변경해서 조회합니다.)
SELECT 
    first_name AS 이름,
    last_name AS 성,
    salary AS 급여
...
/*
 오라클은 혿따옴표로 문자를 표현하고, 문자열 안에 혿따옴표를
...
SELECT DISTINCT department_id FROM employees; -- 중복제거하고 종류만 알려줘라
