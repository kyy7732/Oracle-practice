...
BEGIN
    ...
    IF UPDATING THEN -- UPDATING: 오라클에서 제공하는 빌트인 구문, 시스템 자체에서 상태에 대한 내용을 지원하는 빌트인 구문
        v_type := '수정';
    ELSIF DELETING THEN
        v_type := '삭제';
    END IF;
...
BEGIN
    ...
    v_total := :NEW.total;
    v_product_no := :NEW.product_no; -- 위에 SELECT문이랑 같음
    SELECT
        total
...
INSERT INTO order_history VALUES(order_history_seq.NEXTVAL, 202, 1, 100, 100000);
